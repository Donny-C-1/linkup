    ...
    async function register(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                authStore.login({ id: data.id, username: data.username });
...
<main>
    ...
    <h2>Log in</h2>
    <p class="title">Log in to continue to LinkUp</p>
    <form action="">
        ...
        <div class="input_box">
            ...
            <input type="password" name="password" id="password" bind:value={password} placeholder="Enter your password" autocomplete="current-password" />
        </div>
        <label for="remember_me"><input type="checkbox" name="remember_me" id="remember_me"> Remember me</label>
        <button class="submit" onclick={register}>Log In</button>
    </form>
    <p class="signup_link">Don't have an account? <a href="/signup">Sign up now</a></p>
    <footer>
